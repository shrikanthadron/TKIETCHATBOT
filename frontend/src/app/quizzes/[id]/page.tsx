...
export default function QuizPage() {
  ...
  const fromSubjects = searchParams.get("from") === "subjects";
        ...
        <DashboardLayout>
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="max-w-2xl mx-auto glass-card text-center">
            ...
            {result.newAchievements && result.newAchievements.length > 0 && (
              <div className="mt-6 p-4 rounded-xl bg-amber-500/10 border border-amber-500/30 text-left">
                <p className="font-semibold text-amber-600 dark:text-amber-400 mb-2">New achievements!</p>
            ...
            <button
              type="button"
              onClick={() => router.push(fromSubjects ? "/subjects" : "/quizzes")}
              className="btn-primary inline-block mt-8"
            >
              {fromSubjects ? "Back to Subjects" : "Back to Quizzes"}
            </button>
          </motion.div>
        </DashboardLayout>
      </ProtectedRoute>
